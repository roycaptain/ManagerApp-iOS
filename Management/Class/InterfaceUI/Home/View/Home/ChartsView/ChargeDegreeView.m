...
@implementation ChargeDegreeView
...
-(void)updateWithChargeDegreeModel:(ChargeDegreeModel *)model
{
    ...
    [self setContentWithText:[NSString stringWithFormat:@"%ld",(long)model.totalCycles]];
    ...
    @{
      ...
      @"stops": @[@[@0,@"#CC66FF"],
                  @[@1,@"#66CCFF"]]//颜色字符串设置支持十六进制类型和 rgba 类型
      };
...
{
    NSMutableAttributedString *firstAttri = [[NSMutableAttributedString alloc] initWithString:@"累计充电人次(个): "];
